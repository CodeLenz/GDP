...
#
function Inicializa_Malha_Gmsh(nome_arquivo::String,nnos::Int64,nelems::Int64,
                               connec::Array{Int64,2},coord::Array{Float64,2},
                               dimensao=2)
    ...
    for i=1:nelems
        con = string(i)*" "*string(tipo_elemento)*" 0 "*string(connec[i,1])
        for j=2:size(connec,2)
            con = con * " " * string(connec[i,j])
        end
        println(saida,con)
    end
